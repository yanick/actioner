...
test('shorthands are expanded', () => {
    const actioner = new _index2.default();
...
test('includes', () => {
    ...
    actioner.add('foo', (0, _jsonSchemaShorthand.object)({
        bar: 'number'
    }));

    expect(actioner.schema).toHaveProperty('definitions.foo.properties.included');
});
...
test('definitions', () => {
    ...
    expect(actioner.schema).toHaveProperty('definitions.foo.properties.included.$ref');

    expect(() => actioner.validate({ type: 'FOO', included: "potato" })).toThrow();

    expect(() => actioner.validate({ type: 'FOO', included: true })).not.toThrow();
});